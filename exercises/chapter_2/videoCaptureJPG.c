...
static void  processImage( void *start, int width, int height, int imageSize)
{
    printf("processImage %d, %d size: %d\n", width, height, imageSize);
    for(int i = 0; i < 20; i++)
        printf("%d ",((char *)start)[10000+i]);
    printf("\n");
}
...
/* Step 3: Check supported formats */
    ...
    if(!jpgFound)
    {
        printf("JPEG format not supported\n");
        exit(EXIT_FAILURE);
    }
...
/* Step 8: Allocate a descriptor for each buffer and request its
   ...
   size of the buffers are recorded in the buffers descriptors. */
    bufferDsc *buffers = calloc(reqBuf.count, sizeof(bufferDsc));
...
/* Step 13: Do image processing */
        processImage( buffers[buf.index].start, width, height, buf.bytesused);
